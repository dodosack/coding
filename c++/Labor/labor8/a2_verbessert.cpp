...
public:
    // Standardkonstruktor
    DVD(string title, string publisher, int year_of_publication, int play_time);
...
// hier Konstruktor und Methoden
DVD::DVD(string title = "", string publisher = "", int year_of_publication = 0, int play_time = 0)
...
public:
    // Standardkonstruktor
    Library(int maximal_number_of_media);
...
Library::Library(int maximal_number_of_media = 1000)
    : _c_maximal_number_of_media(maximal_number_of_media) {}

void Library::procure_medium(Medium& a) {

    if (media.size() < _c_maximal_number_of_media) {
        media.push_back(a.clone());
